...
// เวอร์ชันข้อความล้วน (ไม่มี HTML) ของ diff ด้านบน — ใช้ในไฟล์รายงาน Excel
function buildDiffText(oldVal, newVal) {
  const changes = [];
  Object.keys(PRODUCT_FIELD_LABELS).forEach(key => {
    const ov = oldVal[key];
    const nv = newVal[key];
    if (String(ov ?? '') !== String(nv ?? '')) {
      changes.push(`${PRODUCT_FIELD_LABELS[key]}: ${ov ?? '-'} → ${nv ?? '-'}`);
    }
  });
  return changes.join('; ') || 'ไม่มีการเปลี่ยนแปลงค่า';
}

// สรุปข้อมูลสินค้าที่เพิ่มใหม่แบบข้อความล้วน — ใช้ในไฟล์รายงาน Excel (ช่อง "รายละเอียดการเปลี่ยนแปลง" ของแถวที่ action = create)
function buildCreateSummaryText(newVal) {
  const parts = Object.keys(PRODUCT_FIELD_LABELS)
    .map(key => ({ label: PRODUCT_FIELD_LABELS[key], val: newVal[key] }))
    .filter(x => x.val !== null && x.val !== undefined && x.val !== '');
  return parts.map(x => `${x.label}: ${x.val}`).join('; ') || '-';
}
  ...
  try {
    ...
    if (includeProducts) {
      ...
      const rows = (data || []).map(log => {
        const dt = new Date(log.created_at);
        let detail = '-';
        if (log.action === 'create') detail = buildCreateSummaryText(log.new_value || {});
        else if (log.action === 'edit') detail = buildDiffText(log.old_value || {}, log.new_value || {});
        else if (log.action === 'delete') detail = `ลบสินค้า "${log.product_code || ''}"${log.old_value?.product_name ? ' — ' + log.old_value.product_name : ''} ออกจากระบบ`;
        ...
        return {
          ...
          'ชื่อสินค้า': (log.action === 'delete' ? log.old_value?.product_name : log.new_value?.product_name) || '-',
          'ผู้ดำเนินการ': log.username || '-',
          'รายละเอียดการเปลี่ยนแปลง': detail,
        };
      });

      const sheet = XLSX.utils.json_to_sheet(rows.length ? rows : [{ 'ผลลัพธ์': 'ไม่มีข้อมูลในช่วงวันที่ที่เลือก' }]);
      sheet['!cols'] = [{ wch: 12 }, { wch: 8 }, { wch: 12 }, { wch: 18 }, { wch: 28 }, { wch: 16 }, { wch: 70 }];
...
async function onSavePriceRequest(e) {
  ...
  try {
    if (editingPrId) {
      ...
      const { data, error } = await sb
        .from('price_requests')
        .insert({ ...formData, status: 'pending', status_changed_at: new Date().toISOString(), created_by: currentUser.id, updated_by: currentUser.id })
        .select('*')
        .single();
      if (error) throw error;
