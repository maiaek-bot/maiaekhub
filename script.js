...
// ==================================================
// จัดระเบียบข้อความคำขอราคา (ทำงานในเครื่อง/เบราว์เซอร์ทั้งหมด ไม่เรียก API ภายนอก ไม่มีค่าใช้จ่ายใดๆ)
// ดึงจากช่อง "รายละเอียด" มาแยกเป็น สินค้า/สเปค/จำนวน อัตโนมัติแบบ rule-based (จับคำ/รูปแบบที่พบบ่อย)
// พร้อมแนบเช็คลิสต์มาตรฐานที่ควรขอจากผู้ขายทุกครั้ง — เป็นแค่ร่างช่วยร่าง ไม่ใช่ AI จริง ผู้ใช้ควรตรวจทานก่อนส่ง
// ==================================================
...
function organizePriceRequestText(rawText) {
  ...
  const checklist = [
    'รายละเอียดสินค้า (สเปค/วัสดุ/ขนาด) ที่ชัดเจน',
    'รูปตัวอย่างสินค้าจริง',
    'Lead Time (ระยะเวลาส่งสินค้าหลังสั่งซื้อ หรือหลังโอนชำระเงินแล้วกรณีจ่ายเงินสด)',
    'แจ้งด้วยว่ามีสเต็ปราคาตามจำนวนสั่งซื้อหรือไม่ (ถ้ามี ขอราคาทุกสเต็ป)',
  ];
...
function onOrganizePrDetails() {
  const raw = $('fPrDetails').value;
  if (!raw.trim()) {
    showToast('กรุณากรอกรายละเอียดก่อน', 'error');
    return;
  }
  $('prOrganizedText').value = organizePriceRequestText(raw);
      ...
      <div class="pr-card-main">
        ...
        <div class="pr-card-organized">
          <div class="pr-card-organized-head">
            <span class="pr-card-organized-label">✨ สรุปส่งผู้ขาย (ร่างอัตโนมัติ)</span>
          ...
          <pre class="pr-card-organized-text">${escapeHtml(organizePriceRequestText(r.details))}</pre>
        </div>` : ''}
      </div>
...
function onPrListClick(e) {
  ...
  if (action === 'edit') {
    ...
    if (r) copyPlainText(organizePriceRequestText(r.details), btn);
  }
}
