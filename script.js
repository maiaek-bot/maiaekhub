...
function getFilteredSortedProducts() {
  ...
  let filtered = query
    ? allProducts.filter(p => fuzzySearchMatch(query, p, ['product_name', 'product_code', 'items_vendor']))
    : allProducts.slice();
...
function renderPriceRequests() {
  ...
  if (currentPrSearch) {
    filtered = filtered.filter(r => fuzzySearchMatch(currentPrSearch, r, ['details', 'requested_by', 'vendor']));
  }
...
function updateInsightsBadge() {
  ...
  if (!badge) return;
  const count = getStaleProducts().length + getStalePriceRequests().length;
  if (count > 0) {
