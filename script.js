...
function bindPriceRequestEvents() {
  ...
  $('prOrganizeBtn').addEventListener('click', onOrganizePrDetails);
  $('prCopyOrganizedBtn').addEventListener('click', () => {
    const text = $('prOrganizedText').value;
    if (text) copyPlainText(text, $('prCopyOrganizedBtn'));
  });
...
// ==================================================
// จัดระเบียบข้อความคำขอราคาด้วย AI (Gemini ผ่าน serverless function /api/organize-pr — คีย์อยู่ฝั่งเซิร์ฟเวอร์ ไม่มีค่าใช้จ่ายฝั่งผู้ใช้)
// ผลลัพธ์จะถูก cache ไว้ที่ ai_summary ของแต่ละงานขอราคา (สร้างครั้งเดียว, กด "สร้างใหม่" เองถ้าอยากได้ผลลัพธ์ใหม่)
// ถ้าเรียก AI ไม่สำเร็จ (เช่นยังไม่ได้ตั้งค่า API key, เน็ตหลุด, ชน rate limit) จะ fallback ไปใช้ตัวจับ pattern แบบออฟไลน์แทนโดยอัตโนมัติ
// ==================================================
async function organizePriceRequestAI(rawText) {
  const res = await fetch('/api/organize-pr', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ details: rawText }),
  });
  const data = await res.json().catch(() => null);
  if (!res.ok || !data || !data.success) {
    throw new Error((data && data.error) || 'เรียกใช้ AI ไม่สำเร็จ');
  }
  return data.text;
}

// ตัวจับ pattern แบบออฟไลน์ (ทำงานในเบราว์เซอร์ล้วนๆ ไม่มีค่าใช้จ่าย) — ใช้เป็น fallback เมื่อเรียก AI ไม่สำเร็จเท่านั้น
// แม่นยำน้อยกว่า AI มาก เพราะแค่จับคำ/ตัวเลขที่พบบ่อย ไม่ได้ "เข้าใจ" ข้อความจริงๆ
const PR_QTY_UNIT_PATTERN = /(\d+(?:[.,]\d+)?)\s*(คู่|ชิ้น|อัน|แพ็ค|แพค|กล่อง|ตัว|ม้วน|ใบ|เส้น|แผ่น|ชุด|โหล|กก\.?|กิโล|เมตร|pcs?|pack|set|dozen|box|roll)/i;
const PR_QTY_KEYWORD_PATTERN = /(?:ขอ(?:ราคา)?|จำนวน|ต้องการ)\s*(\d+(?:[.,]\d+)?)\s*([ก-๙a-zA-Z]*)/;
const PR_SKIP_LINE_PATTERN = /^(ขอราคา|ขอ\s*\d|จำนวน\s*\d)/;

function extractPrQuantity(text) {
  const unitMatch = text.match(PR_QTY_UNIT_PATTERN);
  if (unitMatch) return `${unitMatch[1]} ${unitMatch[2]}`.trim();

  const keywordMatch = text.match(PR_QTY_KEYWORD_PATTERN);
  if (keywordMatch) return `${keywordMatch[1]} ${keywordMatch[2] || ''}`.trim();

  return '1 (ไม่พบการระบุจำนวนในข้อความ ตั้งค่าเริ่มต้นไว้ที่ 1 — กรุณาตรวจสอบกับผู้ขออีกครั้ง)';
}

function organizePriceRequestFallback(rawText) {
  const text = (rawText || '').trim();
  if (!text) return '';

  const lines = text.split(/\r?\n/).map(l => l.trim()).filter(Boolean);
  const productLine = lines[0] || text.slice(0, 80);
  const specLines = lines.slice(1).filter(l => !PR_SKIP_LINE_PATTERN.test(l));
  const spec = specLines.join(' ') || '-';
  const quantity = extractPrQuantity(text);
  ...
  return [
    ...
    '',
    'สิ่งที่ต้องการจากผู้ขาย: (⚠️ ประมวลผลด้วย AI ไม่สำเร็จ แสดงผลแบบร่างจับ pattern พื้นฐานแทน กรุณาตรวจทานให้ดี)',
    '- ราคา',
  ].join('\n');
}

// ฟังก์ชันกลาง: เรียก AI ก่อน ถ้าพลาดค่อย fallback ไปแบบออฟไลน์ (ใช้ร่วมกันทั้งปุ่มในฟอร์มและปุ่มบนการ์ด)
async function organizePriceRequestSmart(rawText) {
  try {
    return { text: await organizePriceRequestAI(rawText), source: 'ai' };
  } catch (err) {
    console.error('organizePriceRequestAI failed, using offline fallback:', err);
    return { text: organizePriceRequestFallback(rawText), source: 'fallback', error: err.message };
  }
}

async function onOrganizePrDetails() {
  const raw = $('fPrDetails').value;
  if (!raw.trim()) {
    showToast('กรุณากรอกรายละเอียดก่อน', 'error');
    return;
  }
  const btn = $('prOrganizeBtn');
  btn.disabled = true;
  btn.classList.add('is-loading');
  $('prOrganizeOutputWrap').hidden = false;
  $('prOrganizedText').value = 'กำลังให้ AI จัดระเบียบข้อความ…';

  const result = await organizePriceRequestSmart(raw);
  $('prOrganizedText').value = result.text;
  if (result.source === 'fallback') {
    showToast('เรียก AI ไม่สำเร็จ ใช้ตัวจับ pattern สำรองแทนชั่วคราว: ' + result.error, 'error');
  }
  btn.disabled = false;
  btn.classList.remove('is-loading');
}
...
// ==================================================
// สรุป AI ต่อการ์ดในหน้ารายการขอราคา — cache ไว้ที่ ai_summary ของแต่ละแถว (สร้างครั้งเดียว, กด "สร้างใหม่" เองถ้าอยากได้ผลลัพธ์ใหม่)
// ==================================================
const prGeneratingIds = new Set(); // เก็บ id ของงานขอราคาที่กำลังเรียก AI มาจัดระเบียบข้อความอยู่ (กันกดซ้ำ + โชว์สถานะ "กำลังสร้าง...")

function renderPrOrganizedBlock(r) {
  if (prGeneratingIds.has(r.id)) {
    return `
      <div class="pr-card-organized is-loading">
        <div class="pr-card-organized-head">
          <span class="pr-card-organized-label">✨ กำลังให้ AI จัดระเบียบข้อความ…</span>
        </div>
      </div>`;
  }
  ...
  if (r.ai_summary) {
    return `
      <div class="pr-card-organized">
        <div class="pr-card-organized-head">
          <span class="pr-card-organized-label">✨ สรุปส่งผู้ขาย (AI)</span>
          <div class="pr-card-organized-actions">
            <button type="button" class="btn btn-ghost btn-icon" data-action="regenerate-organized" data-id="${r.id}" title="สร้างใหม่" aria-label="สร้างสรุปใหม่">
              <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-2.64-6.36"/><path d="M21 3v6h-6"/></svg>
            </button>
            <button type="button" class="btn btn-ghost btn-icon" data-action="copy-organized" data-id="${r.id}" title="คัดลอกข้อความสรุป" aria-label="คัดลอกข้อความสรุปสำหรับส่งผู้ขาย">
              <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
            </button>
          </div>
        </div>
        <pre class="pr-card-organized-text">${escapeHtml(r.ai_summary)}</pre>
      </div>`;
  }

  return `
    <div class="pr-card-organized pr-card-organized-empty">
      <button type="button" class="btn btn-ghost btn-sm" data-action="generate-organized" data-id="${r.id}">
        <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><path d="M9.5 2 8 4l1.5 2L8 8l1.5 2M14.5 2 16 4l-1.5 2L16 8l-1.5 2"/><path d="m4 20 16-16M4 20l3-1 1-3"/></svg>
        <span>สร้างสรุปสำหรับผู้ขาย (AI)</span>
      </button>
    </div>`;
}

async function generatePrAiSummary(id) {
  const r = allPriceRequests.find(x => x.id === id);
  if (!r || !r.details || !r.details.trim()) return;
  if (prGeneratingIds.has(id)) return; // กันกดซ้ำ/เรียกซ้ำระหว่างกำลังสร้างอยู่

  prGeneratingIds.add(id);
  renderPriceRequests(); // โชว์สถานะ "กำลังสร้าง..." ทันที

  const result = await organizePriceRequestSmart(r.details);
  prGeneratingIds.delete(id);

  // cache ผลลัพธ์ลง DB เฉพาะตอนได้จาก AI จริงเท่านั้น (ไม่ cache ผลลัพธ์สำรองแบบ pattern เพราะคุณภาพต่ำกว่ามาก อยากให้ลองเรียก AI ใหม่ได้เรื่อยๆ จนกว่าจะสำเร็จ)
  if (result.source === 'ai') {
    r.ai_summary = result.text;
    try {
      const { error } = await sb.from('price_requests').update({ ai_summary: result.text }).eq('id', id);
      if (error) throw error;
      writeSessionCache('maiaekhub:cache:priceRequests', allPriceRequests);
    } catch (err) {
      console.error('บันทึก ai_summary ไม่สำเร็จ:', err);
      showToast('สร้างสรุปสำเร็จ แต่บันทึกลงระบบไม่สำเร็จ (ครั้งหน้าอาจต้องกดสร้างใหม่อีกครั้ง)', 'error');
    }
  } else {
    r.ai_summary = result.text; // โชว์ผลลัพธ์สำรองให้ใช้ไปก่อนชั่วคราว (ไม่บันทึกถาวร)
    showToast('เรียก AI ไม่สำเร็จ: ' + (result.error || '') + ' — แสดงร่างสำรองชั่วคราวแทน (ลองกด "สร้างใหม่" อีกครั้งภายหลัง)', 'error');
  }
  ...
  filtered.forEach(r => {
    ...
    card.innerHTML = `
      ...
      <div class="pr-card-main">
        ...
        ${r.details && r.details.trim() ? renderPrOrganizedBlock(r) : ''}
      </div>
...
function onPrListClick(e) {
  ...
  if (action === 'edit') {
    ...
    cyclePrStatus(id);
  } else if (action === 'copy-organized') {
    const r = allPriceRequests.find(x => x.id === id);
    if (r && r.ai_summary) copyPlainText(r.ai_summary, btn);
  } else if (action === 'generate-organized' || action === 'regenerate-organized') {
    generatePrAiSummary(id);
  }
}
...
function openPrModal(prId) {
  ...
  $('prOrganizeOutputWrap').hidden = true;
  $('prOrganizedText').value = '';
...
async function onSavePriceRequest(e) {
  ...
  try {
    ...
    // สร้างสรุป AI ให้อัตโนมัติครั้งเดียวถ้ายังไม่เคยมี (เพิ่มใหม่ หรือแก้ไขแล้วยังไม่เคยสร้างสรุปมาก่อน) — ถ้ามีอยู่แล้วจะไม่ยุ่งด้วย ต้องกด "สร้างใหม่" เอง
    const savedId = editingPrId || (allPriceRequests[0] && allPriceRequests[0].id);
    const savedRow = allPriceRequests.find(r => r.id === savedId);
    if (savedRow && !savedRow.ai_summary) {
      generatePrAiSummary(savedRow.id);
    }
  } catch (err) {
